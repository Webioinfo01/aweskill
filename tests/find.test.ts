...
describe("find command", () => {
  ...
  it("aggregates both providers, dedupes by name, and keeps skills.sh first", async () => {
      ...
      if (url === "https://sciskillhub.org/api/v1/skills/search") {
        ...
        return {
          ...
          json: async () => ({
            results: [
              {
                id: "open-source/research/protein-search",
                name: "protein-search",
                description: "duplicate",
                similarity_score: 90,
              },
              {
                id: "open-source/research/lifesciences-proteomics",
                name: "lifesciences-proteomics",
                description: "Queries proteins",
                similarity_score: 80,
              },
            ],
          }),
    ...
    expect(output).toContain(
      "   install: aweskill store install sciskill:open-source/research/lifesciences-proteomics",
    );
      ...
      vi.fn(async (input: string | URL) => {
        ...
        if (url.startsWith("https://skills.sh/api/search")) {
          ...
          return {
            ok: true,
            json: async () => ({
              skills: [{ id: "owner/repo-a", name: "protein-search", installs: 8800, source: "owner/repo-a" }],
            }),
          };
        }
        if (url === "https://sciskillhub.org/api/v1/skills/search") {
  ...
  it("rejects sciskill-only filters when used against skills-sh", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async () => ({
        ok: true,
        json: async () => ({
          skills: [{ id: "owner/repo", name: "protein-search", installs: 10, source: "owner/repo" }],
        }),
      })),
    );

    await expect(runFind(context, "protein", { provider: "skills-sh", domain: "Life Sciences" })).rejects.toThrow(
  ...
  it("rejects an invalid sciskill domain before making network requests", async () => {
    ...
    await expect(runFind(context, "protein", { provider: "sciskill", domain: "Biology" })).rejects.toThrow(
      'Invalid --domain value: "Biology"',
    );
    expect(fetchMock).not.toHaveBeenCalled();
  });
  ...
  it("rejects an invalid sciskill stage before making network requests", async () => {
    ...
    await expect(runFind(context, "protein", { provider: "sciskill", stage: "Writing" })).rejects.toThrow(
      'Invalid --stage value: "Writing"',
    );
    expect(fetchMock).not.toHaveBeenCalled();
  });
